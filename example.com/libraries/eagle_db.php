...
class Eagle_db{
	...
	function Insert($table,$data){
		$app = $data;
		foreach($app as $k=>$v){
			if(isset($data[$k]) && $data[$k]) $data[$k] = $this->Quote("$v");
			else $data[$k] = "NULL";
		}
		$app = array();
		unset($app);
		$this->db['res']->exec("INSERT INTO ".$table." (".implode(',',array_keys($data)).") VALUES (".implode(',',array_values($data)).")") or die(print_r($this->db['res']->errorInfo(), true));
	}
	
	function Exec($query){
		$this->reset_parts();
		$res = $this->db['res']->prepare($query);
		if($res->execute()) return $res;
		else return false;
	}
	...
	function FieldReturn($query,$field=0){
		$res = $this->Exec($query);
		$ret = $this->Fetch($res);
		return $ret[$field];
	}
	
	function RowReturn($query){
		$res = $this->Exec($query);
		$ret = $this->Fetch($res);
		return $ret;
	}
